  ...
  const dados = [
    {
      ...
      description: 'Valor atual do ativo no par de negociação (ex: BTC/USDT), variação percentual 24h, máxima e mínima do dia.',
      metricas: ['Preço atual', 'Variação 24h (%)', 'Máxima/Mínima 24h']
    },
    {
      icon: 'fas fa-chart-candlestick',
      title: 'Gráficos de Candles',
      description: 'Visualização OHLC (abertura, máxima, mínima, fechamento) de cada período para análise de tendências.',
      metricas: ['Candles 1m, 5m, 1h, 4h, 1d', 'Padrões gráficos', 'Zonas de suporte/resistência']
    },
    {
      icon: 'fas fa-book',
      title: 'Livro de Ordens',
      description: 'Faremos de você um especialista na Leitura de Livro de Ordens, ou se preferir fazemos isto por você.',
      metricas: ['Ordens de compra', 'Ordens de venda', 'Spread bid-ask']
    },
    {
      icon: 'fas fa-layer-group',
      title: 'Profundidade de Mercado',
      description: 'Auxílio em visualização gráfica da quantidade total de compra/venda em cada faixa de preço (depth chart).',
      metricas: ['Volume por nível', 'Paredes de compra/venda', 'Liquidez do mercado']
    },
    {
      icon: 'fas fa-exchange-alt',
      title: 'Volume e Trades',
      description: 'Auxílio com análise de volume negociado em 24h e histórico em tempo real das últimas negociações (preço, quantidade, lado).',
      metricas: ['Volume 24h', 'Trades recentes', 'Lado comprador/vendedor']
    },
    {
      icon: 'fas fa-chart-line',
      title: 'Indicadores Técnicos',
      description: 'Ferramentas de análise técnica básicas: médias móveis (média do preço ao longo do tempo), RSI (indica se está caro ou barato), MACD (mostra mudanças de tendência), Bandas de Bollinger (mostra faixa de preço esperada), etc.',
      metricas: ['Médias móveis', 'RSI', 'MACD', 'Bandas de Bollinger']
    },
    {
      icon: 'fas fa-bullseye',
      title: 'Tipos de Ordem',
      description: 'Diferentes tipos de ordem disponíveis: market, limit, stop-loss, stop-limit, OCO, trailing stop.',
      metricas: ['Market', 'Limit', 'Stop', 'OCO']
    },
      {
        icon: 'fas fa-rocket',
        title: 'Dados de Futuros',
        description: 'Informações específicas de contratos futuros: funding rate, open interest, long/short ratio.',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2 className="text-3xl lg:text-5xl font-extrabold mb-4">
            <span className="bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
              Domine os Dados do Mercado
            </span>
          </h2>
          <p className="text-xl text-gray-400 max-w-3xl mx-auto">
            Aprenda a interpretar e usar todos os dados fornecidos pelas principais corretoras
          </p>
        </div>
          ...
          {dados.map((dado, index) => (
            ...
            >
              ...
              <div className="relative z-10 space-y-4">
                {/* Ícone */}
                <div 
                  className="w-12 h-12 bg-gradient-to-br from-blue-400 to-cyan-400 rounded-lg shadow-lg shadow-blue-500/30 group-hover:scale-110 transition-all duration-300 flex items-center justify-center"
                >
                  {dado.title === 'Preço em Tempo Real' ? (
                    <span className="text-white text-2xl font-bold" style={{ display: 'grid', transform: 'rotate(-6deg)' }}>$</span>
                    ...
                    <svg className="text-white w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                ...
                {dado.title === 'Gráficos de Candles' && (
                  <p className="text-xs text-gray-400 italic">
                    (visualização do preço com abertura, máxima, mínima e fechamento em cada período)
                  </p>
                )}
                ...
                <div className="pt-2 border-t border-blue-400/20">
                  <ul className="space-y-2">
                    {dado.metricas.map((metrica, idx) => {
                      ...
                      if (typeof metrica === 'object' && metrica !== null && 'nome' in metrica && 'descricao' in metrica) {
                        const metricaObj = metrica as { nome: string; descricao: string }
                        return (
                          <li key={idx} className="flex items-start gap-2 text-xs text-gray-400">
                            <Icon name="fas fa-check" className="text-blue-400 text-xs mt-0.5 flex-shrink-0" />
                            <span>
        ...
        <div className="mt-12 text-center">
            <p className="text-lg text-gray-300 mb-4">
              ...
              <span className="text-blue-400 font-semibold">Aprenda como</span>, ou se preferir <span className="text-blue-400 font-semibold">calculamos</span> e <span className="text-blue-400 font-semibold">interpretamos</span> cada uma dessas métricas para você ganhar tempo
            </p>
