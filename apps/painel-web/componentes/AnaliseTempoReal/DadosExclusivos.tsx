  ...
  const exclusivos = [
    {
      ...
      description: 'Sistemas que combinam preço, volume, derivativos, on-chain, sentimento, notícias e muito mais para gerar sinais probabilísticos de alta/baixa te dando alta vantagem competitiva.',
    ...
    },
    {
      ...
      description: 'Métricas como CVD (Cumulative Volume Delta), desequilíbrio do book, agressões de compra/venda e impacto de grandes ordens (whales).',
    ...
    },
    {
      ...
      description: 'MVRV, realized cap/price, NVT e uso real da rede (endereços ativos, volume on-chain) para estimar se o ativo está "caro" ou "barato".',
    ...
    },
    {
      ...
      description: 'Combinações de médias móveis, breakouts, RSI, MACD, ADX otimizadas com backtests sérios para identificar tendências fortes.',
    ...
    },
    {
      ...
      description: 'Cálculos de volatilidade futura, risco de drawdown, tamanho ótimo de posição e alavancagem máxima aceitável para proteção do capital. Alerta inteligente sugerindo troca de um ativo mesmo que você não esteja logado em nossa plataforma.',
    ...
    },
    {
      ...
      description: 'Evolução do valor travado (TVL), uso de dApps, volume em DEX e renda de fees do protocolo para tokens ligados a DeFi/NFT.',
    ...
    },
    {
      ...
      description: 'Índices Fear & Greed, análise de redes sociais (Twitter/X, Reddit, Telegram), fluxo de stablecoins e posicionamento agregado.',
    ...
    },
    {
      ...
      description: 'Listagens em grandes corretoras, alterações regulatórias, atualizações de protocolo, unlocks, hacks, decisões judiciais com impacto estimado.',
    ...
    },
    {
      ...
      description: 'Avaliação automática de time, histórico de entrega, auditorias, governança, tokenomics (distribuição, vesting, inflação/queima) e concentração de holders.',
        ...
        <div className="text-center mb-16">
          <div className="inline-block mb-4">
            ...
            <span className="bg-gradient-to-r from-purple-400 via-blue-400 to-cyan-300 bg-clip-text text-transparent">
              A Informação que o Mercado Não Te Entrega
            </span>
          </h2>
          <p className="text-xl text-gray-400 max-w-3xl mx-auto">
            Dados e análises exclusivas que dão a você uma vantagem competitiva real
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {exclusivos.map((item, index) => (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-gradient-to-br ${item.gradient} rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
              
              {/* Content */}
              <div className="relative z-10 space-y-4">
                {/* Ícone com animação mais elaborada */}
                <div className={`w-14 h-14 bg-gradient-to-br ${item.iconGradient} rounded-xl flex items-center justify-center shadow-xl shadow-blue-500/40 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 glow-blue`}>
                  {item.title === 'Modelos de IA & Machine Learning' ? (
                    ...
                    <svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 20 20">
                      {/* Ondas de fluxo */}
                      <path d="M2 10 Q4 6 6 10 T10 10 T14 10 T18 10" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round"/>
                      <path d="M2 12 Q4 8 6 12 T10 12 T14 12 T18 12" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" opacity="0.7"/>
                      <path d="M2 14 Q4 10 6 14 T10 14 T14 14 T18 14" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" opacity="0.5"/>
                    </svg>
                    ...
                    <svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 20 20">
                      ...
                      {/* Conexões */}
                      <line x1="6" y1="6" x2="10" y2="10" stroke="currentColor" strokeWidth="1" opacity="0.6"/>
                      <line x1="14" y1="6" x2="10" y2="10" stroke="currentColor" strokeWidth="1" opacity="0.6"/>
                      <line x1="6" y1="14" x2="10" y2="10" stroke="currentColor" strokeWidth="1" opacity="0.6"/>
                      <line x1="14" y1="14" x2="10" y2="10" stroke="currentColor" strokeWidth="1" opacity="0.6"/>
                      <line x1="6" y1="6" x2="14" y2="6" stroke="currentColor" strokeWidth="0.8" opacity="0.4"/>
                      <line x1="6" y1="14" x2="14" y2="14" stroke="currentColor" strokeWidth="0.8" opacity="0.4"/>
                    </svg>
                  ) : item.title === 'Modelos de Tendência & Momentum' ? (
                    <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 20 20" strokeWidth="1.5">
                      {/* Círculo do relógio */}
                      ...
                      <line x1="10" y1="10" x2="10" y2="6" stroke="currentColor" strokeWidth="1" strokeLinecap="round"/>
                      {/* Centro do relógio */}
                      <circle cx="10" cy="10" r="1" fill="currentColor"/>
                      {/* Marcadores das horas (12, 3, 6, 9) */}
                      <line x1="10" y1="3" x2="10" y2="4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                      <line x1="17" y1="10" x2="16" y2="10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                      <line x1="10" y1="17" x2="10" y2="16" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                      <line x1="3" y1="10" x2="4" y2="10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                    </svg>
                  ) : item.title === 'Indicadores DeFi & TVL' ? (
                    <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 20 20" strokeWidth="1.5">
                      {/* Cadeado representando valor travado (TVL) */}
                      {/* Corpo do cadeado */}
                      <rect x="6" y="10" width="8" height="6" rx="1" stroke="currentColor" fill="none"/>
                      {/* Arco do cadeado */}
                      <path d="M6 10 Q6 6 10 6 Q14 6 14 10" stroke="currentColor" fill="none" strokeLinecap="round"/>
                      {/* Buraco da fechadura */}
                    ...
                    <svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 20 20">
                      ...
                      <path d="M10 6.5C10 6.5 4.5 1 2 3.5C0.5 5.5 2 8.5 10 16.5C18 8.5 19.5 5.5 18 3.5C15.5 1 10 6.5 10 6.5Z" fill="currentColor" opacity="0.95"/>
                    </svg>
                    ...
                    <svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 20 20">
                      ...
                      <circle cx="10" cy="11" r="4.5" fill="none" stroke="currentColor" strokeWidth="1.5" opacity="1"/>
                      {/* Lente interna - círculo vazado (completamente transparente para mostrar que é uma lente) */}
                      <circle cx="10" cy="11" r="3" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.6"/>
                      {/* Flash no topo */}
                      <rect x="7.5" y="3" width="5" height="3.5" rx="0.5" fill="currentColor" opacity="1"/>
                    </svg>
                  ) : item.title === 'Score de Qualidade' ? (
                    <svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 20 20">
                      {/* Estrela de 5 pontas */}
                      <path d="M 10 2 L 12.5 7.5 L 18.5 8.5 L 14 12.5 L 15.5 18.5 L 10 15 L 4.5 18.5 L 6 12.5 L 1.5 8.5 L 7.5 7.5 Z" fill="currentColor"/>
                    </svg>
                  ) : (
